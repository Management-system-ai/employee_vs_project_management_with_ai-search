...
import { getSkills, updateEmployee } from '@/app/server-actions/supabase/server';
import { deleteEmployeeSkillsByEmployeeId, getEmployeeSkillsById } from '@/app/server-actions/prisma';
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (!validImageTypes.includes(file.type)) {
        console.log('Invalid file type. Please select a valid image (JPEG, PNG, or GIF).');
        return;
      }
      const nameFile = await generateHash(employee?.avatar + new Date().toISOString() )
        ...
        <div className="w-1/2 rounded-lg bg-white p-6">
          ...
          <form>
            <div className="flex justify-center items-center mb-4">
              <div className="relative w-32 h-32">
                <Image
                  ...
                  width={128}
                  height={128}
                  className="rounded-full border border-gray-300 overflow-hidden min-h-full max-h-full max-w-full  object-cover"
                />
                <label
                  htmlFor="avatarUpload"
                  className="absolute bottom-1 right-1 bg-blue-500 text-white p-1.5 rounded-full cursor-pointer shadow-md hover:bg-blue-600 text-sm"
                  title="Update Avatar"
                >
                  <input
                    id="avatarUpload"
            ...
            <div className="mt-6 flex items-end justify-end gap-4">
              ...
              <button
                type="button"
                className={`rounded-lg px-4 py-2 ${loading ? 'bg-gray-400' : 'bg-red-600'
                  } text-white`}
